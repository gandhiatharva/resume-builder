...
const PersonalInfoForm = ({data, onChange, removeBackground, setRemoveBackground}) => {
  ...
  return (
      ...
      <div className='flex items-center gap-2'>
        <label>
            {data.image ? (
                ...
                // if image exist then only we make img tag and create URL for it 
            ) : (
                // if image not avalaible just show the person to upload user image  
            ...
            {/* // selected file is added in the function  */}
        </label>
        {typeof data.image === 'object' && (
            <div className='flex flex-col gap-1 pl-4 text-sm'>
                {/* this is used to remove the background of the image that you have with put, so that you can upload a professional photo  */}
        ...
        return (
            <div key={field.key} className='space-y-1 mt-5'>
                <label className="flex items-center gap-2 text-sm font-medium text-gray-600">
                    ...
                    {/* if required is true show *, else nothing , required is stored with each field  */}
                </label>
    ...
    {/* instead of writing labels and input again and again for form, we can use map with field 
    where each fiels is an array of objects and each object is one input field 
    so for each field we have a label, incon, button  */}
    </div>
  )
}
