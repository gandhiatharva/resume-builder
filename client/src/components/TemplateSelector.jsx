...
const TemplateSelector = ({ selectedTemplate, onChange }) => {
    ...
    // used to tell if we have selected this or not 
  ...
  return (
    <div className='relative'>
        {/* if open it will close, if closed it will open so toggle  */}
      ...
      {isOpen && (
        // if it is open then it needs to show the various templates that we have  
        <div className='absolute top-full w-xs p-3 mt-2 space-y-3 z-10 bg-white rounded-md border border-gray-200 shadow-sm'>
            {templates.map((template)=>(
                <div key={template.id} onClick={()=> {onChange(template.id); setIsOpen(false)}} className={`relative p-3 border rounded-md cursor-pointer transition-all ${selectedTemplate === template.id ?
                    // once we have selected we make it false, so the pop-up is closed 
                    // selected template comes from the props 
...
// this is used to select a template for our frontend, we pass data in resume builder
// which comes and displays information 
